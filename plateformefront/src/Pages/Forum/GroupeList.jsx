...
function GroupeList({ onSelectGroupe, wsConnected }) {  // ✅ Reçoit wsConnected
  ...
  const handleCreateGroupe = async () => {
    ...
    try {
      ...
      setErrorSnackbar(error.response?.data?.message || 'Impossible de créer le groupe');
    } finally {
      setCreating(false);
    }
  };
        ...
        <Box sx={{ maxWidth: 1200, mx: 'auto', px: 3, py: 3 }}>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
            <Box>
              ...
              <Chip 
                label={wsConnected ? '🟢 WebSocket connecté' : '🔴 WebSocket déconnecté'} 
                size="small" 
            ...
            {filteredGroupes.map((groupe) => (
              <Grid item xs={12} sm={6} md={4} key={groupe.id}>
                <Card elevation={2} sx={{ height: '100%', '&:hover': { elevation: 4 } }}>
                  <CardActionArea 
                    onClick={() => handleJoinGroupe(groupe.nom)} 
                    sx={{ height: '100%' }}
                  >
                    <CardContent>
                      <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
                        ...
                        <Box sx={{ flexGrow: 1 }}>
                          ...
                          <Chip 
                            label={`${groupe.nombreMembres} ${groupe.nombreMembres > 1 ? 'membres' : 'membre'}`} 
                            size="small" 
                            sx={{ mt: 0.5 }} 
                          />
                        </Box>
      ...
      >
        ...
        <DialogActions sx={{ px: 3, pb: 2 }}>
          ...
          <Button 
            ...
            variant="contained" 
            disabled={!newGroupeName.trim() || creating}
          >
