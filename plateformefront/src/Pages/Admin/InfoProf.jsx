...
import { api } from "../../ConfigBackEnd/Api";
...
export default function InfoProf() {
  ...
  const fetchDemandes = async () => {
    try {
      setError("");
      // ✅ CORRIGÉ : Bonne URL
      const response = await api.get("/admin/professeurs/demandes");
      ...
      if (error.response?.status === 403) {
        ...
        setError("❌ Erreur lors de la récupération des demandes. Veuillez réessayer.");
      }
    } finally {
      setLoading(false);
    }
  ...
  const validerProfesseur = async (userId) => {
    ...
    try {
      // ✅ CORRIGÉ : Bonne URL
      const response = await api.post(`/admin/professeurs/valider/${userId}`);
      ...
      alert(`✅ ${response.data.message || "Utilisateur validé comme professeur !"}`);
    } catch (error) {
  ...
  const refuserProfesseur = async () => {
    ...
    try {
      // ✅ CORRIGÉ : Bonne URL (pas besoin de raison)
      const response = await api.post(`/admin/professeurs/refuser/${selectedDemande.id}`);
      ...
      alert(`✅ ${response.data.message || "Demande refusée"}`);
    } catch (error) {
      ...
      {showRefusModal && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 animate-in fade-in zoom-in duration-200">
            ...
            <p className="text-sm text-gray-500 mb-6 bg-blue-50 p-3 rounded-lg border border-blue-200">
              ℹ️ L'utilisateur ne sera pas notifié par email du refus.
            </p>
